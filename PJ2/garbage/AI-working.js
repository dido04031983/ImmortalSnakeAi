...
const manhattanMultiWeightFruit=0;
const manhattanPowWeightFruit=1;
const manhattanMultiWeightTail=0;
const manhattanPowWeightTail=1;
const reachablePosMultiWeightFruit=0;
const reachablePosPowWeightFruit=1;
const reachablePosMultiWeightTail=0;
const reachablePosPowWeightTail=1;
const weightFruit=[];
for(let i=0;i<30;i++){
  weightFruit.push(new Float32Array(30));
  for(let j=0;j<30;j++){
    weightFruit[i][j]=1;//0.9-2*(Math.max(Math.abs(14.5-i),Math.abs(14.5-j))-0.5)/35;
  }
}
const weightTail=[];
for(let i=0;i<30;i++){
